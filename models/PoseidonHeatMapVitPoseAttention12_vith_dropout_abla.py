...
class AdaptiveFrameWeighting(nn.Module):
    def __init__(self, embed_dim, num_frames, num_heads=16):
        super(AdaptiveFrameWeighting, self).__init__()
        self.embed_dim = embed_dim
        self.num_frames = num_frames

        self.frame_quality_estimator = nn.MultiheadAttention(embed_dim, num_heads)
    ...
    def forward(self, x):
        ...
        x_flat = x.view(batch_size, num_frames, embed_dim, -1).mean(-1)  # Global Avg Pooling
        x_flat = x_flat.permute(1, 0, 2)  # Shape: [num_frames, batch_size, embed_dim]
        x_flat = self.norm(x_flat)

        attn_output, _ = self.frame_quality_estimator(x_flat, x_flat, x_flat)
        quality_scores = self.fc(attn_output).squeeze(-1).permute(1, 0)  # Shape: [batch_size, num_frames]
...
class MultiScaleFeatureFusion(nn.Module):
    ...
    def forward(self, features):
        ...
        # aggregated features with sum
        fused_features = sum(multi_scale_features.values())
...
class CrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.mha = nn.MultiheadAttention(embed_dim, num_heads, dropout=0.1)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, query, context):
        ...
        attn_output = self.norm_out(attn_output)
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4):
        ...
        #self.adaptive_weighting = AdaptiveFrameWeighting(self.embed_dim, self.num_frames, num_heads=self.num_heads)

        # Cross-Attention
        self.cross_attention = CrossAttention(self.embed_dim, self.num_heads)

        # Self-Attention
        self.self_attention = nn.MultiheadAttention(self.embed_dim, num_heads)

        self.dropout = nn.Dropout(0.1)

        # Layer norms for intermediate features
        self.intermediate_layer_norms = nn.ModuleDict({
            name: self.backbone.ln1 if name == 'layer11' else nn.LayerNorm(self.embed_dim)

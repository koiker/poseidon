...
class DynamicCrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.mha = nn.MultiheadAttention(embed_dim, num_heads)
        self.conv1x1 = nn.Conv2d(embed_dim, embed_dim, kernel_size=1)
        self.dropout = nn.Dropout(0.1)
    ...
    def compute_motion_weights(self, center_frame, context_frames):
        B, num_context_frames, C, H, W = context_frames.shape
        center_frame_expanded = center_frame.unsqueeze(1).expand(-1, num_context_frames, -1, -1, -1)
        
        # Concatenate center frame with each context frame
        combined = torch.cat([center_frame_expanded, context_frames], dim=2)
        combined = combined.view(B * num_context_frames, C * 2, H, W)
        
        # Compute motion weights
        motion_weights = self.motion_estimator(combined)
        motion_weights = motion_weights.view(B, num_context_frames, 1, H, W)
    ...
    def forward(self, query, context):
        ...
        # Compute motion-based weights
        motion_weights = self.compute_motion_weights(query, context)
        
        # Apply motion weights to context frames
        context_weighted = context * motion_weights
        
        query = query.view(B, C, -1).permute(2, 0, 1)
        context_weighted = context_weighted.view(B, num_context_frames, C, -1).permute(3, 0, 1, 2).reshape(-1, B, C)

        # Apply convolution for simple feature transformation
        query = self.conv1x1(query.permute(1, 2, 0).reshape(B, C, H, W))
        query = query.view(B, C, -1).permute(2, 0, 1)

        attn_output, _ = self.mha(query, context_weighted, context_weighted)
        attn_output = self.dropout(attn_output)
        attn_output = attn_output.permute(1, 2, 0).view(B, C, H, W)
        
        return attn_output, motion_weights.squeeze(2)
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4, embed_dim=384):
        ...
        self.cross_attention = DynamicCrossAttention(self.embed_dim, self.num_heads)
    ...
    def forward(self, x, meta=None):
        ...
        attended_features, _ = self.cross_attention(center_frame, context_frames)

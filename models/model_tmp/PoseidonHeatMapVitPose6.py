...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=6, embed_dim_for_joint=30):
        ...
        # Store the FFNs from the backbone
        self.ffn3 = self.backbone.layers[3].ffn
        self.ffn7 = self.backbone.layers[7].ffn
        self.ffn11 = self.backbone.layers[11].ffn
        self.ln1 = self.backbone.ln1
    ...
    def forward(self, x, meta=None):
        ...
        def hook_fn(layer_index):
            def hook(module, input, output):
                intermediate_outputs[layer_index] = output
            return hook

         # Register hooks for layers 3, 7, and 11
        self.backbone.layers[3].register_forward_hook(hook_fn(3))
        self.backbone.layers[7].register_forward_hook(hook_fn(7))
        self.backbone.layers[11].register_forward_hook(hook_fn(11))

        # Forward pass through the backbone
        self.backbone(x)

        # Apply FFN to each intermediate output
        output3 = self.ffn3(intermediate_outputs[3])
        output7 = self.ffn7(intermediate_outputs[7])
        output11 = self.ffn11(intermediate_outputs[11])

        # Sum the outputs
        combined_output = output3 + output7 + output11

        print("Shape of combined output: ", combined_output.shape)

        # Apply the final layer normalization
        x = self.ln1(combined_output)

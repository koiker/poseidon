...
def generate_temporal_attention_mask(seq_length, window_size):
    # Create a mask with zeros in the allowed attention positions and -inf elsewhere
...
class AdaptiveFrameWeighting(nn.Module):
    ...
    def forward(self, x):
        ...
        quality_scores = self.frame_quality_estimator(x_reshaped)  # [batch_size * num_frames, 1]
        quality_scores = quality_scores.view(batch_size, num_frames)  # [batch_size, num_frames]
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_mask = generate_temporal_attention_mask(num_frames, self.window_size).to(x.device)

        x = self.transformer_encoder(x, mask=attention_mask)  # Apply attention mask
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4, embed_dim=384, num_layers=2):
        ...
        print(f"Extracted layers: {self.return_layers}")

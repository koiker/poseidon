...
class PyramidPoolingModule(nn.Module):
    def __init__(self, in_channels, out_channels, pool_sizes=[1, 2, 3, 6]):
        ...
        self.avg_pool_paths = nn.ModuleList()
        self.max_pool_paths = nn.ModuleList()

        for pool_size in pool_sizes:
            self.avg_pool_paths.append(nn.Sequential(
                nn.AdaptiveAvgPool2d(pool_size),
                nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False),
                nn.BatchNorm2d(out_channels),
                nn.ReLU(inplace=True)
            ))
            self.max_pool_paths.append(nn.Sequential(
    ...
    def forward(self, x):
        ...
        features = [x]
        for avg_pool, max_pool in zip(self.avg_pool_paths, self.max_pool_paths):
            #features.append(F.interpolate(avg_pool(x), size=(h, w), mode='bilinear', align_corners=True))
            #features.append(F.interpolate(max_pool(x), size=(h, w), mode='bilinear', align_corners=True))

            avgpool_feature = F.interpolate(avg_pool(x), size=(h, w), mode='bilinear', align_corners=True)
            maxpool_feature = F.interpolate(max_pool(x), size=(h, w), mode='bilinear', align_corners=True)

            # sum over channels
            features.append(avgpool_feature + maxpool_feature) 

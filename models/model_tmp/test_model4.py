...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4, embed_dim=1280):
        ...
        # Layer normalization
        self.layer_norm = nn.LayerNorm([embed_dim, 16, 12])
    ...
    def forward(self, x, meta=None):
        ...
        for feature_name in intermediate_outputs.keys():
            ...
            print(f"Intermediate output {feature_name} shape: {intermediate_outputs[feature_name].shape}")
            intermediate_outputs[feature_name] = intermediate_outputs[feature_name].view(batch_size, num_frames, self.embed_dim, 16, 12)
        ...
        intermediate_outputs['model_output'] = intermediate_outputs['model_output'].view(batch_size, num_frames, self.embed_dim, 16, 12)
        ...
        x = x.view(batch_size, num_frames, self.embed_dim, 16, 12) # [batch_size, num_frames, 384, 24, 18]  
        ...
        context_frames = context_frames.view(-1, self.embed_dim, 16*12).permute(2, 0, 1)
        context_frames, _ = self.self_attention(context_frames, context_frames, context_frames)
        context_frames = context_frames.permute(1, 2, 0).view(batch_size, num_frames-1, self.embed_dim, 16, 12)
...
def test_model():
    ...
    # load weights
    weights_path = ""# "/home/pace/Poseidon/results/2024-08-03_1/best_model.pt"
    ...
    dummy_input = torch.randn(2, 5, 3, 256, 192).to(device)
...
# python models/test_model.py --config /home/pace/Poseidon/configs/configPoseidon.yaml

...
class StackedCrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers=3):
        super(StackedCrossAttention, self).__init__()
        self.layers = nn.ModuleList([CrossAttention(embed_dim, num_heads) for _ in range(num_layers)])
        self.norm = nn.LayerNorm(embed_dim)  # Add layer norm for stability
    ...
    def forward(self, query, context):
        for layer in self.layers:
            ...
            attn_output = layer(query, context)
            
            # Residual connection and normalization for attention output
            query = self.norm(query + attn_output)
            
            # Apply feedforward network to refine features
            ffn_output = self.ffn(query)
            
            # Add the residual connection for the FFN as well
            query = self.norm(query + ffn_output)
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4, embed_dim=1280):
        ...
        self.return_layers = {'layers.9': 'layer9','layers.15': 'layer15', 'layer.21': 'layer21',}
        ...
        # Cross-Attention
        self.cross_attention = StackedCrossAttention(self.embed_dim, self.num_heads)

...
class AttentionFusion(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        # Self-Attention with Dropout Layers
        ...
        self.dropouts = nn.ModuleList([nn.Dropout(0.1) for _ in range(3)])
    ...
    def forward(self, features):
        ...
        # Apply self-attention layers sequentially with dropout
        for attn_layer, dropout in zip(self.attention_layers, self.dropouts):
            attn_output, _ = attn_layer(features_cat, features_cat, features_cat)
            attn_output = dropout(attn_output)
            features_cat = attn_output  # update the features for the next layer
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, context):
        ...
        context = context.reshape(-1, B, C) # [H*W*num_context_frames, B, C]
         ...
         # Apply attention layers sequentially with dropout
        for attn_layer, dropout in zip(self.attention_layers, self.dropouts):
            query, _ = attn_layer(query, context, context)
            query = dropout(query)
            attn_output = query  # update the features for the next layer
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4, embed_dim=384,  dropout_prob=0.1):
        ...
        # Feature Fusion
        self.feature_fusion = AttentionFusion(self.embed_dim, num_heads=self.num_heads)
    ...
    def forward(self, x, meta=None):
        ...
        # Apply each Self-Attention layer with dropout
        for self_attention_layer, dropout in zip(self.self_attention_layers, self.dropouts):
            context_frames, _ = self_attention_layer(context_frames, context_frames, context_frames)
            context_frames = dropout(context_frames)

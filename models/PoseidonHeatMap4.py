...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase=TRAIN_PHASE, num_heads=5, embed_dim_for_joint=30):
        ...
        self.self_attention_layers = nn.ModuleList([
            nn.Sequential(
                nn.MultiheadAttention(embed_dim=self.embed_dim_for_joint*4, num_heads=self.num_heads, batch_first=True),
                nn.Dropout(p=0.1)
            ) for _ in range(3)
        ])
    ...
    def forward(self, x, meta=None):
        ...
        x = x.permute(0, 1, 3, 2)
        x = x.reshape(-1, self.num_joints, self.embed_dim_for_joint*4)

        for self_attention_layer in self.self_attention_layers:
            x, _ = self_attention_layer[0](x, x, x)  # Apply attention
            x = self_attention_layer[1](x)  # Apply dropout

...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=6, embed_dim_for_joint=30):
        ...
        # Add a channel attention mechanism
        self.channel_attention = ChannelAttention(384)
    ...
    def forward(self, x, meta=None):
        ...
        x = self.spatial_pool(backbone_outputs)  # shape: [batch_size*num_frames, 384, 1, 1]

        # Apply channel attention
        x = self.channel_attention(x)

        # print("After adaptive pooling:", x.shape) # torch.Size([batch_size*num_frames, 384, 2, 2])

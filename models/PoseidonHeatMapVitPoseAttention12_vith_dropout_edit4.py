...
class MultiScaleFeatureFusion(nn.Module):
    def __init__(self, embed_dim, num_heads, num_features):
        ...
        self.attention_fusion = AttentionFusion(embed_dim, num_heads, num_features)
...
class AttentionFusion(nn.Module):
    def __init__(self, embed_dim, num_heads, num_features):
        ...
        self.fusion_attention = nn.MultiheadAttention(embed_dim * num_features, num_heads)
        self.norm = nn.LayerNorm(embed_dim*num_features)
        self.final_proj = nn.Linear(num_features * embed_dim, embed_dim)  # Updated projection layer
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, features):
        ...
        for key, value in features.items():
            # Reshape and collect features
            value = value.reshape(B * num_frames, embed_dim, H * W).permute(2, 0, 1)
            feature_list.append(value)

        # Apply attention to each feature separately (optional)
        attn_outputs = []
        for feature in feature_list:
            attn_output, _ = self.attention(feature, feature, feature)
            attn_outputs.append(attn_output)

        # Concatenate features
        fused_features = torch.cat(attn_outputs, dim=2)  # Shape: [sequence_length, batch_size*num_frames, num_features * embed_dim]

        # Apply the additional attention layer
        attn_fused, _ = self.fusion_attention(fused_features, fused_features, fused_features)

        # Continue with layer norm, dropout, and projection as before
        fused_features = self.norm(attn_fused)
        fused_features = self.dropout(fused_features)
        fused_features = self.final_proj(fused_features)

        # Reshape back to spatial dimensions
        fused_features = fused_features.permute(1, 2, 0).view(B * num_frames, embed_dim, H, W)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, context):
        B, C, H, W = query.shape
        num_context_frames = context.shape[1]

        # Flatten spatial dimensions and add positional encodings
        query = query.view(B, C, H * W).permute(2, 0, 1)  # Shape: [H*W, B, C]
        #query = query + self.positional_encoding.unsqueeze(1).to(query.device)  # Add positional encoding

        # Prepare context
        # Reshape context to have batch size N = B
        # Shape: [num_context_frames * H * W, B, C]
        context = context.view(B, num_context_frames, C, H * W)  # [B, num_context_frames, C, H*W]
        context = context.permute(0, 2, 1, 3).contiguous()       # [B, C, num_context_frames, H*W]
        context = context.view(B, C, num_context_frames * H * W) # [B, C, num_context_frames * H*W]
        context = context.permute(2, 0, 1)                       # [S, B, C], where S = num_context_frames * H*W

        # Apply cross-attention
        attn_output, _ = self.mha(query, context, context)       # Shapes: query[L, N, E], context[S, N, E]
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4, embed_dim=1280):
        ...
        # Feature Fusion
        num_features = len(self.return_layers) + 1  # +1 for 'model_output' if included

        # Update the feature fusion module
        self.feature_fusion = MultiScaleFeatureFusion(self.embed_dim, num_heads=self.num_heads, num_features=num_features)

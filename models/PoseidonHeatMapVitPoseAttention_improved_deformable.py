...
class DeformableCrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dim_heads=64,offset_groups=4, downsample_factor=4):
        super(DeformableCrossAttention, self).__init__()
        # Deformable attention instead of standard multi-head attention
        self.deformable_attention = DeformableAttention2D(
            dim=embed_dim,
            heads=num_heads,
            dim_head=dim_heads,
    ...
    def forward(self, query, context):
        ...
        query = query.view(B, C, H, W)  # central frame
        context = context.view(B * num_context_frames, C, H, W)  # context frames
        
        # Apply deformable attention
        attn_output = self.deformable_attention(query, return_vgrid=False)

        return attn_output


class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=8, embed_dim=384):
        ...
        # Cross-Attention
        self.cross_attention = DeformableCrossAttention(self.embed_dim, self.num_heads)

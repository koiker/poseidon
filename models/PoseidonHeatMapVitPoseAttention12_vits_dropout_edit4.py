...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_heads=4):
        ...
        # Cross-Attention
        self.cross_attention_layers = nn.ModuleList([CrossAttention(self.embed_dim, self.num_heads) for _ in range(self.num_attention_layers)])

        # Additional Layer Norms for Cross Attention Layers
        self.attention_layer_norms = nn.ModuleList([nn.LayerNorm([self.embed_dim, 24, 18]) for _ in range(self.num_attention_layers)])
    ...
    def forward(self, x, meta=None):
        ...
        # Stacked Cross-Attention Layers with Residual Connections
        for i in range(self.num_attention_layers):
            # Cross-Attention
            attended_features = self.cross_attention_layers[i](center_frame, context_frames)

            # Layer Norm after each attention layer
            attended_features = self.attention_layer_norms[i](attended_features)

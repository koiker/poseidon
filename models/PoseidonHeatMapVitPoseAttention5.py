...
class SpatialAttentionLayer(nn.Module):
    def __init__(self, embed_dim, output_size=(6, 4)):
    ...
    def forward(self, x):
        ...
        # Reduce spatial dimensions, but not completely
        x_pooled = self.pooling(x.view(-1, embed_dim, H, W))
        pooled_H, pooled_W = x_pooled.shape[-2:]
        
        # Compute attention weights
        attention_weights = self.attention(x_pooled)

        # Reshape attention weights
        attention_weights = attention_weights.view(batch_size, num_frames, embed_dim, pooled_H, pooled_W)
        
        # Upsample attention weights to original spatial dimensions
        attention_weights = F.interpolate(attention_weights.view(-1, embed_dim, pooled_H, pooled_W), 
                                          size=(H, W), mode='bilinear', align_corners=False)
        attention_weights = attention_weights.view(batch_size, num_frames, embed_dim, H, W)
...
class Poseidon(nn.Module):
    def __init__(self, cfg, device='cpu', phase='train', num_attention_layers=5, embed_dim=384):
        ...
        self.attention_layers = nn.ModuleList([SpatialAttentionLayer(embed_dim) for _ in range(num_attention_layers)])
